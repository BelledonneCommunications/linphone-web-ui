...
jQuery.i18n.translate_key = 'translate';
...
jQuery.i18n.update = function(element, recursive) {
	...
	element.filter(function () { return $(this).metadata()[jQuery.i18n.translate_key] != null}).each(function() {
		var element = $(this);
		element.find('.'+jQuery.i18n.translate_class).remove();
		element.append(jQuery.i18n.translate(element.metadata()[jQuery.i18n.translate_key]));
	})
	if(recursive) {
		element.find('*').filter(function () { return $(this).metadata()[jQuery.i18n.translate_key] != null}).each(function() {
			var element = $(this);
			element.find('.'+jQuery.i18n.translate_class).remove();
			element.append(jQuery.i18n.translate(element.metadata()[jQuery.i18n.translate_key]));
		})
...
jQuery.i18n.translate = function(text) {
	...
	var ret = jQuery('<span class="' + jQuery.i18n.translate_class + '"/>').text(translated_text);
	return ret;
};

jQuery.i18n.get = function(text) {
	var translated_text = jQuery.i18n.translate(text);
	return jQuery('<div class="{'+jQuery.i18n.translate_key+': \'' + text + '\'}" />').append(translated_text);
};